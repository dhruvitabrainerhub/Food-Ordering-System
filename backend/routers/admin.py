...
def admin_user(credentials: HTTPAuthorizationCredentials = Depends(security), db: Session = Depends(get_db)):
...
def add_restaurant(data: RestaurantCreate, db: Session = Depends(get_db), user=Depends(admin_user)):
...
def toggle_restaurant(rid: int, db: Session = Depends(get_db), user=Depends(admin_user)):
...
def delete_restaurant(rid: int, db: Session = Depends(get_db), user=Depends(admin_user)):
    r = db.query(Restaurant).filter(Restaurant.id == rid).first()
    if not r: raise HTTPException(status_code=404, detail="Not found")
    db.delete(r)
    db.commit()
...
def add_menu_item(data: MenuItemCreate, db: Session = Depends(get_db), user=Depends(admin_user)):
...
def delete_menu_item(item_id: int, db: Session = Depends(get_db), user=Depends(admin_user)):
...
def update_order_status(order_id: int, status: str, db: Session = Depends(get_db), user=Depends(admin_user)):
